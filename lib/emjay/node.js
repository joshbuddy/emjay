...
Emjay.NodeJs.Instance.prototype = {
  runWithMethod: function(bindings, callback, previousGenerator, method) {
    ...
    posix.cat(instance.file).addCallback(function(content) {
      ...
      stage2.run(function(stage3) {
        puts("stage3: "+stage3);
        stage3.previousStage3 = stage3;
        stage3.__factory = stage3.factory;
  ...
  run: function(bindings, callback, previousGenerator) {
    this.runWithMethod(bindings, callback, previousGenerator, 'run');
  }
};
...
Emjay.NodeJs.Helpers = {
  fileToString: function(file) {
    ...
    posix.cat(file).addCallback(function (content) {
      sys.debug("--CONTENT "+content);
      var safeContent = content.replace(/'/g,"\\'").replace(/\[-/g,'\\[-').replace(/-\]/g,'-\\]').replace(/\n/g,'\\n');
      sys.puts("--SAFE CONTENT:"+safeContent);
      __runtime.write("'"+safeContent+"'");
      __runtime.done();
    });
  },
  partial: function(string, bindings) {
    var stage3 = new Emjay.Stage3(this.previousStage3.stage2, bindings, string);
