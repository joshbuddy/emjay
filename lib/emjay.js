...
Emjay.Parser.prototype = {
  init: function() {
    ...
    for(var index in parts) {
      ...
      puts(index+"->working on part: "+part);
      if (part.substr(0,3) == '[==') {
        var p = "this.compiled.push(function(__result){" + part.substr(3, part.length - 5) + "});";
        puts(p)
        eval(p);
      } else if (part.substr(0,2) == '[=') {
        var p = "this.compiled.push(function(result){ result.done('__runtime.__buffer += __runtime.htmlEscape(" + part.substr(2, part.length - 4).toJSON().replace(/'/g, "\\\'") + ");\\n');});";
        puts(p)
        eval(p);
      } else if(part.substr(0,2) == '[-') {
        var p = "this.compiled.push(function(result){ result.done('" + part.substr(2, part.length - 4).toJSON().replace(/'/g, "\\\'") + ";\\n');});";
        puts(p)
        eval(p);
      } else if(part == '') {
      // or, this is easy, just append it!
      } else {
        eval("this.compiled.push(function(result){ result.done('__runtime.__buffer += \"" + part.toJSON().replace(/\n/g, '\\\n') + "\";\\n')});");
      }
    }
    puts('done compiling');
  },
...
Emjay.Generator.prototype = {
  ...
  append: function(output) {
    this.write("__runtime.__buffer += __runtime.htmlEscape('" + output.toString().toJSON().replace(/'/g, "\\\'") + "');\n");
    return this;
  },
  rawAppend: function(output) {
    this.write("__runtime.__buffer += '" + output.toString().toJSON().replace(/'/g, "\\\'") + "';\n");
    return this;
  },
