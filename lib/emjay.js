...
Emjay.Parser.prototype = {
  ...
  generator: function(bindings, previousGenerator) {
    return new Emjay.Generator(this, bindings, previousGenerator);
  },
};
...
Emjay.Generator.prototype = {
  ...
  ancestor: function() {
    return this.previousGenerator ? this.previousGenerator.ancestor() : this;
  }
};
...
Emjay.Runtime.prototype = {
  ...
  append: function(s) {
    if (s === undefined) {
      // do nothing..
    } else if (this.__generator.isArray(s)) {
      for (var i in s) {
        this.append(s[i]);
      }
    } else if(typeof(s) == 'object' && s.safe === undefined) {
  ...
  unprocessedResult: function() {
    var prog = '';
    __runtime = this;
    if (this.__bindings !== undefined) {
      for(var i in this.__bindings) {
        prog += "var " + i + " = __runtime.__bindings['" + i.toJSON() + "'];\n";
      }
    }
    return prog + this.__program;
  },
  result: function() {
    this.__buffer = [];
    var finalProgram = this.unprocessedResult();
    eval(finalProgram);
    return this.bufferToString(this.__buffer);
  },
